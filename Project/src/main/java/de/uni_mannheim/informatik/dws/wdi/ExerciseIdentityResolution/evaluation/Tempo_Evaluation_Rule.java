...
public class Tempo_Evaluation_Rule extends EvaluationRule<Song, Attribute> {
    @Override
    public boolean isEqual(Song record1, Song record2, Attribute schemaElement) {

        if(record1.getTempo() == -1 && record2.getTempo() == -1)
            return true;
        else if(Math.abs(record1.getTempo() - record2.getTempo()) <= 5)
            return true;
        else
            return false;
    }
